...
typedef struct {
	Color *col;
	size_t collen;
	Font font, bfont, ifont, ibfont, efont;
	GC gc;
} DC;

static inline ushort sixd_to_16bit(int);
static int xisemoji(Rune);
...
xloadfonts(const char *fontstr, double fontsize)
{
	...
	epattern = FcNameParse((const FcChar8 *)emoji_font);
	if (!epattern)
		die("can't open emoji font %s\n", emoji_font);
	FcPatternAddDouble(epattern, FC_PIXEL_SIZE, usedfontsize);
	if (xloadfont(&dc.efont, epattern))
		die("can't open emoji font %s\n", emoji_font);
	FcPatternDestroy(epattern);
}
...
xisemoji(Rune rune)
{
	return BETWEEN(rune, 0x2300, 0x27bf) ||
	       BETWEEN(rune, 0x2b00, 0x2bff) ||
	       BETWEEN(rune, 0x1f000, 0x1faff) ||
	       rune == 0x200d || rune == 0xfe0f || rune == 0x20e3;
}
...
{
	int i, x, ox, numspecs, baseemoji = 0, newemoji;
	Glyph base, new;
	XftGlyphFontSpec *specs = xw.specbuf;
	...
	for (x = x1; x < x2; x++) {
		...
		if (new.mode == ATTR_WDUMMY)
			continue;
		newemoji = xisemoji(new.u);
		if (selected(x, y1))
			new.mode ^= ATTR_REVERSE;
		if ((i > 0) && (ATTRCMP(base, new) || baseemoji != newemoji)) {
