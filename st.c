...
#define TLINE(y)		((y) < term.scr ? term.hist[((y) + term.histi - \
	            term.scr + 2 * HISTSIZE + 1) % HISTSIZE] : \
...
typedef struct {
	...
	int histlen;  /* lines currently in history */
	int scr;      /* scroll back */
...
selactive(void)
{
	return sel.ob.x != -1 && sel.alt == IS_SET(MODE_ALTSCREEN);
}
...
selsnap(int *x, int *y, int direction)
{
	...
	case SNAP_WORD:
		...
		for (;;) {
			...
			if (!BETWEEN(newx, 0, term.col - 1)) {
				...
				if (!BETWEEN(newy, term.scr - term.histlen,
				             term.scr + term.row - 1))
					break;
	...
	case SNAP_LINE:
		...
		*x = (direction < 0) ? 0 : term.col - 1;
		if (direction < 0) {
			for (; *y > term.scr - term.histlen; *y += direction) {
			...
			for (; *y < term.scr + term.row - 1; *y += direction) {
...
tscrolldown(int orig, int n, int copyhist)
{
	...
	if (copyhist) {
		...
		term.histlen = MIN(term.histlen + 1, HISTSIZE);
